...
if(isset($_POST['url']))
{
	...
	$url = str_replace(array("http://", "http://www.", "www.", "https://", "https://www."), array('', '', '', '', ''), $url);
	if(strlen($url) < 3) die("Cole ou digite a URL, por favor.");
	if(url_exist($url))
	{
		//check if exists
		...
		if(@mysql_num_rows($rs)) {
			...
			if($rs) {
				...
				print "Não foi possível criar link encurtado".mysql_error();
			}
		}
